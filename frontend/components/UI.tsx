  ...
  return (
    <aside className="w-64 bg-slate-900 text-white flex flex-col shadow-2xl z-20 hidden md:flex">
      ...
      <div className="p-4 border-t border-slate-800">
        <div className="flex items-center gap-3 px-4 py-2 cursor-pointer hover:bg-slate-800 rounded-xl transition-all">
          ...
          <div className="text-sm">
            ...
            <p className="text-slate-500 text-xs">admin@docuquest.ai</p>
          </div>
        </div>
      </div>
    </aside>
...
export function Header() {
  ...
  return (
    <header className="h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center px-8 z-10 sticky top-0 justify-between">
      <h2 className="text-slate-800 font-semibold text-lg">
        {pathname === '/upload' ? 'Process Document' : pathname.includes('/document/') ? 'Extraction Review' : 'Workspace Overview'}
      </h2>
    </header>
  );
}
